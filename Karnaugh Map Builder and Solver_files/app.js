...
function HighlightColor( Value )
{
	...
	if (Value=="0") return "rgb(50,120,160)"; //~0xFF0000;
...
function GenerateTruthTableHTML()
{
	var Text = "<table class=\"table-view\" ID=\"TruthTableID\" style=\"text-align:center; width: 90%;margin-left: 5%;\">";
	{
		...
		for (i=0; i<TruthTable.length; i++)
		{
			...
			{
				if (DisplayValue(TruthTable[i][j].Variable) == 0) {
					var color = "style=\"background-color: rgba(255,255,255,.5);\""
				}else{
					var color = "";
				}
			...
			Text = Text
				+ "<td><input class=\"btn-kmap\" ID=\""+TruthTable[i].ButtonUIName +"\" name="+TruthTable[i].ButtonUIName +" type='button' value='"+DisplayValue(TruthTable[i].KMapEntry.Value)+"' onClick=\"ToggleTTEntry(TruthTable["+i+"])\" ></td>" 
				+ "</tr>";
		}
	}
	Text = Text + "</table><div style=\"height: 1rem;\"></div>";
	return Text;
}
...
function GenerateKarnoMapHTML()
{
	...
	{
		Text += "<th class=\"header-color\" style=\"\">"+BinaryString(BitOrder[i],KMap.XVariables)+"</th>";
	}
	Text+="</tr>";
	...
	for (h=0; h<KMap.Height; h++)
	{
		...
		if (h==0)
		{
			Text += "<th style=\"width: 10%;font-weight: 900; text-align: center; color:#228090;border-right: 3px solid #228090;\" rowspan="+((KMap.Height) + 2)  +">";
			for (i=0; i<KMap.YVariables; i++)
			{
				Text += "<b class=\"header-color\">" + VariableNames[i+KMap.XVariables] + "</b>";
			}
		}
		Text += "<th class=\"header-color\" style=\"text-align: center;width: 10%\" >"+BinaryString(BitOrder[h],KMap.YVariables)+"</th>";

		for (w=0; w<KMap.Width; w++)
		{

			Text += "<td  ID='"+KMap[w][h].TDUIName+"' style='text-align:center;'>"
					+ "<input class=\"btn-kmap\" ID="+KMap[w][h].ButtonUIName +" name="+KMap[w][h].ButtonUIName +" type='button'  value='"+DisplayValue(KMap[w][h].Value)+"' onClick=\"ToggleKMEntry(KMap["+w+"]["+h+"])\">"
					+ "</td>";
		}
		Text += "</tr>";
	}
